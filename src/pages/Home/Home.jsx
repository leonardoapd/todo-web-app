    ...
    <>
      <main className='home'>
        <section className='app__home-container'>
          ...
          <button className='prev' onClick={() => setCurrentIndex((prevIndex) => (prevIndex - 1 + homeInfo.length) % homeInfo.length)}>
            &#10094;
          </button>
          <button className='next' onClick={() => setCurrentIndex((prevIndex) => (prevIndex + 1) % homeInfo.length)}>
            &#10095;
          </button>
        </section>
        ...
        <div style={{ textAlign: 'center' }}>
          {homeInfo.map((item, index) => (
            <span className={`dot ${index === currentIndex ? 'active-dot' : ''}`} key={index} onClick={() => setCurrentIndex(index)}></span>
          ))}
        </div>
      </main>
    </>
